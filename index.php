                ...
                for (let i = 0; i < slice.length; ++i) {
                    ...
                    xhttp.onreadystatechange = function() {
                        if (this.readyState == 4 && this.status == 200) {
                            ...
                            p_short_text = '';
                            page.querySelector('div#content').querySelectorAll('p').forEach((element) => {
                                p_short_text += element.innerText + ' '
                            });

                            // обрезаем до максимальной длины
                            var trimmedString = p_short_text.substr(0, maxLength);
                            // переобрезаем, если оказались посреди слова
                            trimmedString = trimmedString.substr(0, Math.min(trimmedString.length, trimmedString
                                .lastIndexOf(" ")));
